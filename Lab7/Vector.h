...
public:
	//коснтруктор с параметрами выделяет память под s элементов и заполняте их значениями k
	Vector(int s, T k);
	...
	//оперция для добавления константы
	Vector operator+(const T k);
	//операция возвращающая длину вектора
	int operator()();
	//перегружение операции ввода-вывода
	//<> - указывают на то, что ф-я является шаблоном
...
{
	Vector<T> temp(size, k); //инициализурет вреенный вектор любым знач
	for (int i = 0; i < size; i++)
